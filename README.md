...
Per **interfacciarsi con l'utente**, `Simulation` utilizza un'interfaccia di tipo `IMenu`. Le classi che implementano `IMenu` correttamente (come `CommandLineMenu`) possono essere utilizzate dare degli ordini alla simulazione in base allo stato in cui si trova. I metodi da implementare per rendere il menù funzionante sono:
